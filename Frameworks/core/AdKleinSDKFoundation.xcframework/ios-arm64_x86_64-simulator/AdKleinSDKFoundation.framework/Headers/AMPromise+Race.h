...
@interface AMPromise<Value>(RaceAdditions)
...
/**
 Convenience dot-syntax wrappers for `AMPromise` `race` operators.
 Usage: AMPromise.race(@[ ... ])
 */
@interface AMPromise<Value>(DotSyntax_RaceAdditions)

+ (AMPromise * (^)(NSArray *))race AM_PROMISES_DOT_SYNTAX NS_SWIFT_UNAVAILABLE("");
+ (AMPromise * (^)(dispatch_queue_t, NSArray *))raceOn AM_PROMISES_DOT_SYNTAX
