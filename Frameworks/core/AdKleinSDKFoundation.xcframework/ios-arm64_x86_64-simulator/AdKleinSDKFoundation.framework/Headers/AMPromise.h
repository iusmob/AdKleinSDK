 ...
 */
@interface AMPromise<__covariant Value> : NSObject

/**
 Default dispatch queue used for `AMPromise`, which is `main` if a queue is not specified.
 */
...
#ifdef AM_PROMISES_DOT_SYNTAX_IS_DEPRECATED
#define AM_PROMISES_DOT_SYNTAX __attribute__((deprecated))
#else
#define AM_PROMISES_DOT_SYNTAX
#endif

@interface AMPromise<Value>(DotSyntaxAdditions)

/**
 Convenience dot-syntax wrappers for AMPromise.
 Usage: AMPromise.pending()
        AMPromise.resolved(value)

 */
+ (instancetype (^)(void))pending AM_PROMISES_DOT_SYNTAX NS_SWIFT_UNAVAILABLE("");
+ (instancetype (^)(id __nullable))resolved AM_PROMISES_DOT_SYNTAX NS_SWIFT_UNAVAILABLE("");
